...
| `COVERAGE_GATE_MODE` | Modo do gate de cobertura das fichas no job `cobertura` do `data-freshness-audit` | Vem da variável de repositório de mesmo nome; ausente, vale `warn` (só avisa). Trocar para `enforce` depois de duas execuções com `GATE_SEM_RECIBO cells=0`. Outro valor faz o job falhar. | GitHub Actions (variável de repositório) |
...
| `PF_PREVIOUS_VERSION` | Versão que precisa estar no topo do ledger de migrations antes do apply da quarentena ampliada de gastos | Obrigatória no apply que a lê; precisa ser a migration imediatamente anterior em ordem de arquivo, senão o apply aborta. | Input do workflow |
...
| `CHECAGENS_SLUGS`, `CHECAGENS_GRAVAR` | Recorte e modo da busca nominal de checagens (`checagens-coleta.yml`) | Derivadas dos inputs `slugs` e `gravar` do disparo manual. Na execução agendada o recorte é vazio (cadastro inteiro) e a gravação no `coleta_log` fica ligada; no disparo manual o padrão é dry-run. | GitHub workflow |
