...
| `PF_PERMITIR_TMP` | Coletor judicial `curadoria-processos-lote.ts --coorte-atual` | Opcional; `1` libera evidência e snapshot em `/tmp`. Ausente, o coletor recusa `/tmp` porque o reboot de 24/09 apagou o snapshot de agosto. | Teste local |
| `APLICAR`, `MARGEM_DIAS` | Variáveis internas do job de `processos-coleta-judicial.yml` | Derivadas do evento e dos inputs do workflow: `APLICAR` é `true` no agendamento e no disparo manual com `aplicar`; `MARGEM_DIAS` vem do input (padrão 5). Não configurar como secret nem na Vercel. | Workflow |
...
| Variáveis | Contexto | Obrigatoriedade e fallback | Responsável |
|---|---|---|---|
| `ROSTER_DIR`, `ROSTER_PISO_TOTAL` | Workflow `roster-deputados.yml` | Definidas no próprio job: `ROSTER_DIR` é a pasta efêmera dos pacotes TSE baixados; `ROSTER_PISO_TOTAL` (19000) é o mínimo de linhas do dry-run para liberar o upsert. Sem fallback: abaixo do piso o job falha sem escrever. | Workflow `roster-deputados.yml` |
...
| `pesquisas-monitoramento.yml` | 10:17 UTC diária e manual | Coleta e verificação das pesquisas eleitorais da matriz aprovada (`verify:pesquisas`). Dívida conhecida: falha fechada em toda execução agendada desde 22/09/2026 e não publica; o run vermelho traz a anotação com o motivo e a issue `cron-failure` acompanha. A atualização viva de Presidente, Governador e Senado é a importação manual auditada (`npm run pesquisas:importar`, `docs/operations/pesquisas-importacao-manual.md`). |
| `roster-deputados.yml` | Terça e sexta, 08:41 UTC e manual | Baixa `consulta_cand` e `consulta_cand_complementar` do TSE, roda o dry-run de `scripts/ingest-roster-deputados.ts`, exige cobertura provada e piso de 19.000 linhas e só então faz upsert em `candidatos_roster_2026` (`--include-majoritarios --apply`). Dispatch manual grava só com `apply=true`. Fotos continuam nulas: pacote e URL de imagem do TSE responderam 403 em 25/09/2026. |
...
| `processos-coleta-judicial.yml` | Segunda e quinta, 09:17 UTC; manual (dry-run por padrão) | Renova recibos judiciais `processos-curadoria` com 9+ dias (SLA de 14 do site) e busca candidatos públicos sem recibo (DJEN e DataJud); grava só recibos em `coleta_log` pelo aplicador auditado, com backup e readback. Falha vira recibo `erro` por alvo, com tipo de falha fechado; `erro` é renovado na execução seguinte. Grupo de concorrência `ingest-pipeline`, sem cancelar execução em curso. `check-processos-receipts.ts` antes e depois; resumo sem dados nominais; backup do runner descartado (coleta_log é append-only). Nunca grava em `processos`. |
