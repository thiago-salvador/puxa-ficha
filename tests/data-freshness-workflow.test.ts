...
test("snapshot força leitura e auditoria preserva quatro artefatos", () => {
  ...
  assert.match(sql, /latest_collection_runs/i)
  assert.match(sql, /log\.execucao IS NOT DISTINCT FROM latest\.execucao/i)
  assert.match(sql, /debt_count/i)
  assert.doesNotMatch(sql, /bool_or\(resultado IN \('erro', 'indeterminado'\)\)/i)
