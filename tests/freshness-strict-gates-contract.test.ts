...
test("workflow deixa recuperação manual no gate da coorte e exige opt-in para strict-all", () => {
  ...
  assert.match(surfaceSection, /npm run audit:nucleo-fichas-publicas/)
  assert.match(surfaceSection, /public-profile-core-report\.json/)
  assert.match(surfaceSection, /npm run audit:contrato-fichas-publicas/)
  assert.match(surfaceSection, /public-profile-contract\.json/)
