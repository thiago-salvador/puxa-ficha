...
test("forward readback e harness PG17 separam apply e rollback", () => {
  ...
  assert.match(PG17_HARNESS, /invoke_apply\(\)/)
  assert.doesNotMatch(PG17_HARNESS, /python3 - "\$DDL_HASH" "\$BACKFILL_HASH"/)
  ...
  assert.match(PG17_HARNESS, /bash scripts\/audit\/apply-issue-138-production\.sh/)
  assert.match(PG17_HARNESS, /PF_DATABASE_URL=.*db\.wskpzsobvqwhnbsdsmok\.supabase\.co/)
  assert.match(PG17_HARNESS, /PF_EXPECTED_SHA=\"\$RUNNER_EXPECTED_SHA\"/)
  assert.match(PG17_HARNESS, /GITHUB_REF=\"\$ref\"/)
  assert.match(PG17_HARNESS, /PF_TEST_REMOTE_SHA/)
  assert.match(PG17_HARNESS, /SHA remoto fail-closed/)
  ...
  assert.match(PG17_HARNESS, /trap 'limpar INT' INT/)
  assert.match(PG17_HARNESS, /trap 'limpar TERM' TERM/)
  assert.match(PG17_HARNESS, /trap 'limpar EXIT' EXIT/)
  assert.doesNotMatch(PG17_HARNESS, /trap cleanup_wrappers/)
  assert.match(PG17_HARNESS, /INT\) rc=130/)
  assert.match(PG17_HARNESS, /TERM\) rc=143/)
  assert.match(PG17_HARNESS, /docker rm -fv "\$CONTAINER_ID"/)
  assert.match(PG17_HARNESS, /docker rm -fv "\$CONTAINER_ID"[^\n]*remove_rc=\$\?/)
  assert.match(PG17_HARNESS, /CONTAINER_ID="\$\(docker run -d/)
  assert.match(PG17_HARNESS, /trap - INT TERM EXIT/)
  assert.match(PG17_HARNESS, /CONTAINER_VOLUMES=/)
  assert.match(PG17_HARNESS, /docker volume inspect "\$volume"/)
