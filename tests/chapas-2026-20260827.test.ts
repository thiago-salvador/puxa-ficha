...
const profileLinks = JSON.parse(readFileSync(linksPath, "utf8")) as {
  ...
  links: Array<{ sq_candidato: string; slug: string; exists_production: boolean }>
}

test("snapshot de 27/08 congela o universo oficial completo e sem PII", () => {
  assert.equal(snapshot.metadata.source_sha256, "eae2178d1d87c6f66c81ac5c6a56f10118a0bff373068135531315cec6f74a27")
  ...
  assert.equal(snapshot.chapas.filter((row) => row.identidade_status === "duplicidade_oficial").length, 19)
  assert.equal(snapshot.chapas.filter((row) => row.identidade_status === "confirmada").length, 201)
  assert.ok(snapshot.chapas.every((row) => row.titular.sq_candidato && row.titular.perfil_slug))
...
test("auditoria consegue representar os 427 registros oficiais sem duplicar titulares", () => {
  ...
  const activeSp = snapshot.chapas.find((row) => row.vice.sq_candidato === "250002552372")
  const replacedSp = snapshot.chapas.find((row) => row.vice.sq_candidato === "250002544911")
...
test("vínculos reaproveitam 44 fichas exatas e criam somente duas", () => {
  assert.equal(profileLinks.metadata.source_sha256, snapshot.metadata.source_sha256)
  assert.equal(profileLinks.links.length, 46)
  assert.equal(profileLinks.links.filter((link) => link.exists_production).length, 44)
  assert.deepEqual(
    profileLinks.links.filter((link) => !link.exists_production).map((link) => link.slug).sort(),
    ["rico-pinheiro", "well-macedo"],
  )
  assert.equal(new Set(profileLinks.links.map((link) => link.sq_candidato)).size, 46)
})
...
test("forward, rollback e readback são fail-closed", () => {
  ...
  assert.match(migration, /baseline exata com 196 chapas e a inclusão posterior de Pablo Marçal/)
