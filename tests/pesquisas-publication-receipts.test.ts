...
for (const mode of ["curadoria", "ausente", "hash", "challenge", "origem", "timeout", "desconhecido", "inventário", "listagem", "artefato inválido"]) {
  test(`CLI de consolidação: ${mode}`, () => {
    ...
    try {
      ...
      const discovery = { status: "partial", source_filter: "all", coverage, intake: { entries: [] }, observations: LISTAGENS_PESQUISAS.map(row => ({ id: row.id, status: mode === "listagem" ? "unavailable" : "observed", evidence_sha256: sha("listing fixture"), observed_at: "2026-09-12T12:00:00Z", error: null })), inventory: { geographies: coverage.map(row => ({ geography_code: row.geography_code, status: "observed", query_exhausted: mode !== "inventário", errors: [], records: [], pages: [{}] })) } }
      ...
      assert.equal(run.status, mode === "curadoria" ? 0 : 1, run.stderr || run.stdout)
      ...
      assert.equal(result.execution_status, mode === "curadoria" ? "complete" : "failed")
