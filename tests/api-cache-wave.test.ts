...
test("toda unstable_cache pública inclui o ponto único de bump", () => {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === "unstable_cache") {
  ...
  for (const call of cacheCalls) {
    ...
    assert.ok(ts.isArrayLiteralExpression(key), "unstable_cache deve declarar uma chave estática")
    assert.ok(
