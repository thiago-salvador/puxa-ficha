...
test("driver conserva guardas do predecessor antes de chamar o compositor ou psql", () => {
  ...
  assert.ok(driver.includes(guard), "guardas de main e SHA devem ser mantidas literalmente")
  const tlsStart = previous.indexOf("[[ \"$database_ref\"")
  const tlsEnd = previous.indexOf("\nversion=20260905150000")
  assert.ok(tlsStart >= 0 && tlsEnd > tlsStart)
  assert.ok(driver.includes(previous.slice(tlsStart, tlsEnd)), "projeto de producao e libpq/TLS devem ser mantidos literalmente")
