...
BEGIN
  IF (SELECT count(*) FROM public.coleta_log WHERE execucao='migration:20260916160000')<>1
     OR (SELECT count(*) FROM public.coleta_log WHERE execucao='rollback:20260916160000')<>1
     OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260916160000') THEN
  ...
  SELECT detalhe::jsonb INTO b FROM public.coleta_log WHERE execucao='rollback:20260916160000';
  IF (SELECT to_jsonb(x) FROM public.candidatos x WHERE id='d45f1947-73a7-4292-9955-7e57927032f0') IS DISTINCT FROM b->'candidato'
     ...
     OR b->'patrimonio'->>'despublicacao_motivo' IS DISTINCT FROM 'Rollback da migration 20260916160000'
