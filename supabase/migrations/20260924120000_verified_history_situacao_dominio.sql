-- A RPC do histórico verificado validava a situação do registro contra uma
-- lista fixa de sete valores, escrita em 08/09/2026. Em 16/09/2026 o domínio de
-- candidatos.situacao_candidatura ganhou 'pendente de julgamento'
-- (DS_SITUACAO_JULGAMENTO código 17), e a lista da RPC ficou para trás: toda
-- observação desse valor passou a falhar com 'Invalid verified registration
-- status' e derrubava a coleta semanal do histórico.
--
-- A lista aceita passa a ser exatamente o domínio de
-- candidatos_situacao_candidatura_dominio, na mesma ordem de
-- SITUACAO_CANDIDATURA_DOMINIO em src/lib/situacao-candidatura.ts. Um teste
-- compara as três listas; mudou uma, mudam as outras na mesma PR. Valores fora
-- do domínio continuam rejeitados com o mesmo erro.
--
-- Mesma assinatura, mesmo SECURITY DEFINER, mesmo search_path e mesmos grants
-- da definição original; só a lista da situação muda. CREATE OR REPLACE mantém
-- dono e ACL, e os grants são reafirmados abaixo iguais aos da origem.
BEGIN;
...
BEGIN
  ...
  END IF;
  IF has_function_privilege('anon', 'public.observe_verified_candidate_change(uuid,text,integer,text,text,text)', 'EXECUTE')
    OR has_function_privilege('authenticated', 'public.observe_verified_candidate_change(uuid,text,integer,text,text,text)', 'EXECUTE')
    OR NOT has_function_privilege('service_role', 'public.observe_verified_candidate_change(uuid,text,integer,text,text,text)', 'EXECUTE') THEN
