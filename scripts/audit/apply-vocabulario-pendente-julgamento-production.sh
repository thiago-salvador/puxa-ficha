#!/usr/bin/env bash
# Aplica 20260916130000 (alargamento do CHECK de situacao_candidatura para
# 'pendente de julgamento') com predecessor, hash, lock, ledger e readback
# fechados para o projeto de producao do Puxa Ficha.
#
# NAO aplicar sem o lado TypeScript na MESMA revisao: SITUACAO_CANDIDATURA_DOMINIO
# em src/lib/situacao-candidatura.ts e o espelho deste CHECK, e
...
previous_version=20260916120000
migration="$ROOT/supabase/migrations/${version}_vocabulario_situacao_pendente_julgamento.sql"
previous_migration="$ROOT/supabase/migrations/${previous_version}_verified_candidate_updates_security_invoker.sql"
rollback="$ROOT/supabase/rollback/${version}_vocabulario_situacao_pendente_julgamento.rollback.sql"
readback="$ROOT/supabase/readback/${version}_vocabulario_situacao_pendente_julgamento.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato do alargamento do vocabulario de situacao_candidatura (pendente de julgamento) ausente" >&2
  exit 2
}
# O digest do predecessor e CALCULADO do arquivo, nunca copiado para dentro
# deste script: um literal aqui seria uma segunda copia do mesmo hash, livre
# para divergir em silencio da migration real.
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: vocabulario de pendente de julgamento ja alargado, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial do alargamento do vocabulario de pendente de julgamento inesperado: $state" >&2
  exit 1
fi
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:vocabulario-pendente-julgamento-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'vocabulario pendente de julgamento: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'vocabulario pendente de julgamento: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: vocabulario de pendente de julgamento aplicado, ledger e readback concluidos"
