#!/usr/bin/env bash
# Aplica 20260903220000 (despublicacao logica de 6 linhas de historico_politico e
# 2 de financiamento que pertencem a ALVARO FERNANDES DIAS (PR) e estao na ficha
# de ALVARO COSTA DIAS (RN)) com predecessor, hash, lock, ledger e readback
...
previous_version=20260903210000
migration="$ROOT/supabase/migrations/${version}_despublicar_alvaro_dias_rn_homonimo.sql"
previous_migration="$ROOT/supabase/migrations/${previous_version}_vocabulario_situacao_julgamento_publicado.sql"
rollback="$ROOT/supabase/rollback/${version}_despublicar_alvaro_dias_rn_homonimo.rollback.sql"
readback="$ROOT/supabase/readback/${version}_despublicar_alvaro_dias_rn_homonimo.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato da despublicacao do homonimo de alvaro-dias-rn ausente" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: despublicacao do homonimo de alvaro-dias-rn ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial da despublicacao do homonimo de alvaro-dias-rn inesperado: $state" >&2
  exit 1
fi
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:alvaro-dias-rn-homonimo-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'alvaro-dias-rn homonimo: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'alvaro-dias-rn homonimo: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: despublicacao do homonimo de alvaro-dias-rn aplicada, ledger e readback concluidos"
