#!/usr/bin/env bash
# Prova em PostgreSQL 17 descartavel de 20260916130000 (alargamento do CHECK de
...
# o oitavo valor E recusando um nono, adulteracao derrubando o readback,
# migration posterior bloqueando o rollback, rollback RECUSANDO quando ja
# existe linha com o estado novo gravado, rollback limpo, CHECK de sete
# valores voltando a morder, e no-op de replay quando o dominio nunca foi
# instalado.
set -euo pipefail
...
VERSION="20260916130000"
PREVIOUS="20260916120000"
MIGRATION="supabase/migrations/${VERSION}_vocabulario_situacao_pendente_julgamento.sql"
READBACK="supabase/readback/${VERSION}_vocabulario_situacao_pendente_julgamento.readback.sql"
ROLLBACK="supabase/rollback/${VERSION}_vocabulario_situacao_pendente_julgamento.rollback.sql"
ROLLBACK_READBACK="supabase/readback/${VERSION}_vocabulario_situacao_pendente_julgamento.rollback.readback.sql"
...
# Estado pos-20260903210000: dominio de SETE valores instalado.
q -q <<SQL
...
INSERT INTO public.candidatos(slug, situacao_candidatura) VALUES
  ('ruth-reis', 'aguardando julgamento'),
...
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: readback aceitou o pre-estado (dominio de sete valores)" >&2; exit 1
fi
...
# O OITAVO valor passa a ser aceito.
q -q -c "UPDATE public.candidatos SET situacao_candidatura='pendente de julgamento' WHERE slug='ficha-declarada'" \
  || { echo "FAIL: CHECK alargado recusou 'pendente de julgamento'" >&2; exit 1; }
# E um nono estado, que a fonte nao emite para esta coorte, continua barrado.
...
# Rollback tem de RECUSAR enquanto existe o estado novo gravado.
if q -q < "$ROLLBACK" >/dev/null 2>&1; then
  echo "FAIL: rollback aceitou estreitar o dominio com linha em 'pendente de julgamento'" >&2; exit 1
fi
...
# Adulteracao 2: CHECK presente, mas incompleto (perdeu 'indeferido com recurso').
q -q -c "ALTER TABLE public.candidatos ADD CONSTRAINT candidatos_situacao_candidatura_dominio CHECK (situacao_candidatura IN ('aguardando julgamento', 'candidatura declarada', 'incerto', 'deferido', 'deferido com recurso', 'indeferido', 'pendente de julgamento'))"
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: readback aceitou CHECK sem 'indeferido com recurso'" >&2; exit 1
fi
q -q -c "ALTER TABLE public.candidatos DROP CONSTRAINT candidatos_situacao_candidatura_dominio"
q -q -c "ALTER TABLE public.candidatos ADD CONSTRAINT candidatos_situacao_candidatura_dominio CHECK (situacao_candidatura IN ('aguardando julgamento', 'candidatura declarada', 'incerto', 'deferido', 'deferido com recurso', 'indeferido', 'indeferido com recurso', 'pendente de julgamento'))"
q -q < "$READBACK"

q -q -c "INSERT INTO supabase_migrations.schema_migrations(version, idempotency_key) VALUES ('20260917000000', 'sha256:future')"
if q -q < "$ROLLBACK" >/dev/null 2>&1; then
  echo "FAIL: rollback aceitou migration posterior" >&2; exit 1
fi
q -q -c "DELETE FROM supabase_migrations.schema_migrations WHERE version='20260917000000'"

q -q < "$ROLLBACK"
q -q < "$ROLLBACK_READBACK"

# O CHECK de sete valores volta a morder.
voltou="$(docker exec -i "$CONTAINER_ID" psql -X -U postgres -d postgres -Atq -c "UPDATE public.candidatos SET situacao_candidatura='pendente de julgamento' WHERE slug='ficha-declarada'" 2>&1 | grep -c 'violates check constraint "candidatos_situacao_candidatura_dominio"' || true)"
[[ "$voltou" == "1" ]] || { echo "FAIL: apos o rollback o CHECK de sete valores nao rejeitou 'pendente de julgamento'" >&2; exit 1; }
...
echo "PASS: alargamento do vocabulario para 'pendente de julgamento' tem forward sem tocar dado, o oitavo estado aceito, um nono barrado, CHECK incompleto reprovado, rollback recusado sobre estado gravado, rollback limpo, CHECK de sete valores remordendo e no-op de replay provados em PostgreSQL 17"
