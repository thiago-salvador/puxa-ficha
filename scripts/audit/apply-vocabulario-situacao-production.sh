#!/usr/bin/env bash
# Aplica o PAR 20260903100000 (dado) + 20260903100100 (CHECK) de situacao_candidatura,
# numa unica transacao, com predecessor, hash, lock, ledger e readback fechados
# para o projeto de producao do Puxa Ficha.
set -euo pipefail
...
previous_version=20260902200000
previous_digest=sha256:15418497551ce486a7d77c429783e4a647ed8a4dc61f712ec34c0a5f369228c7
migration_a="$ROOT/supabase/migrations/${version_a}_vocabulario_situacao_candidatura.sql"
migration_b="$ROOT/supabase/migrations/${version_b}_vocabulario_situacao_candidatura_check.sql"
rollback="$ROOT/supabase/rollback/${version_b}_vocabulario_situacao_candidatura.rollback.sql"
readback="$ROOT/supabase/readback/${version_b}_vocabulario_situacao_candidatura.readback.sql"
[[ -f "$migration_a" && -f "$migration_b" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato do vocabulario de situacao_candidatura ausente" >&2
  exit 2
}

digest_a="sha256:$(shasum -a 256 "$migration_a" | cut -d' ' -f1)"
digest_b="sha256:$(shasum -a 256 "$migration_b" | cut -d' ' -f1)"
state="$(PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  ...
  "select coalesce(max(version),'') || '|' || count(*) filter(where version='$previous_version') || '|' || coalesce(max(idempotency_key) filter(where version='$previous_version'),'') || '|' || count(*) filter(where version='$version_a') || '|' || coalesce(max(idempotency_key) filter(where version='$version_a'),'') || '|' || count(*) filter(where version='$version_b') || '|' || coalesce(max(idempotency_key) filter(where version='$version_b'),'') from supabase_migrations.schema_migrations")"
IFS='|' read -r ledger_top previous_count previous_key a_count a_key b_count b_key <<<"$state"

if [[ "$ledger_top" == "$version_b" && "$a_count" == "1" && "$a_key" == "$digest_a" && "$b_count" == "1" && "$b_key" == "$digest_b" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: vocabulario de situacao_candidatura ja aplicado, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$a_count" != "0" || "$b_count" != "0" ]]; then
  echo "FAIL: ledger inicial do vocabulario de situacao_candidatura inesperado: $state" >&2
  exit 1
fi

python3 - "$PF_EXPECTED_SHA" "$version_a" "$version_b" "$previous_version" "$digest_a" "$digest_b" "$previous_digest" "$migration_a" "$migration_b" "$rollback" "$readback" <<'PY' | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -
import base64, pathlib, re, sys

sha, version_a, version_b, previous, digest_a, digest_b, previous_digest, path_a, path_b, rollback_path, readback_path = sys.argv[1:]
...
raw_a, body_a = split_body(path_a)
raw_b, body_b = split_body(path_b)
rollback = pathlib.Path(rollback_path).read_bytes()
readback = pathlib.Path(readback_path).read_bytes()
name_a = pathlib.Path(path_a).stem.removeprefix(version_a + "_")
name_b = pathlib.Path(path_b).stem.removeprefix(version_b + "_")
created_by = "Thiago Salvador <contato.thiagosalvador@gmail.com> via github-actions:" + sha

def ledger_row(version, name, digest, raw):
    return (
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:vocabulario-situacao-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version IN ({lit(version_a)}, {lit(version_b)})) THEN RAISE EXCEPTION 'vocabulario situacao: ledger divergiu sob lock'; END IF; END $ledger$;")
print(body_a, end="" if body_a.endswith("\n") else "\n")
print(ledger_row(version_a, name_a, digest_a, raw_a))
print(body_b, end="" if body_b.endswith("\n") else "\n")
print(ledger_row(version_b, name_b, digest_b, raw_b))
print(readback.decode("utf-8"), end="" if readback.endswith(b"\n") else "\n")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version_b)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version_a)} AND idempotency_key={lit(digest_a)}) <> 1 OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version_b)} AND idempotency_key={lit(digest_b)}) <> 1 THEN RAISE EXCEPTION 'vocabulario situacao: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: vocabulario de situacao_candidatura aplicado (par), ledger e readback concluidos"
