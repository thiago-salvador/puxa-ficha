...
node --import tsx scripts/audit/apply-vice-status.ts "$mode" "$PF_EXPECTED_SHA" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -v VERBOSITY=terse -f -
if [[ "$mode" == "apply" ]]; then
  node --import tsx scripts/audit/apply-vice-status.ts verify "$PF_EXPECTED_SHA" | \
...
echo "PASS: vice-status mode=$mode"
