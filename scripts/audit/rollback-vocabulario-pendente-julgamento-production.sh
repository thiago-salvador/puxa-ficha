#!/usr/bin/env bash
# Reverte 20260916130000 reinstalando o CHECK de sete valores. Recusa quando ja
# existe linha gravada com 'pendente de julgamento': estreitar o dominio nesse
# caso apagaria fato publicado pelo TSE.
set -euo pipefail
...
rollback_readback="$ROOT/supabase/readback/${version}_vocabulario_situacao_pendente_julgamento.rollback.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$rollback_readback" ]] || {
  echo "FAIL: artefato de rollback do alargamento do vocabulario de pendente de julgamento ausente" >&2
  exit 2
}
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'rollback vocabulario pendente de julgamento: ledger divergiu sob lock'; END IF; END $ledger$;")
print(body, end="" if body.endswith("\n") else "\n")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'rollback vocabulario pendente de julgamento: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY
...
echo "PASS: rollback do vocabulario de pendente de julgamento concluido, CHECK de sete valores reinstalado"
