#!/usr/bin/env bash
# Aplica 20260903200000 (backfill de sq_candidato_2026 em well-macedo e
# rico-pinheiro) com predecessor, hash, lock, ledger e readback fechados para o
...
previous_version=20260903140000
migration="$ROOT/supabase/migrations/${version}_backfill_sq_candidato_ondas_agosto.sql"
previous_migration="$ROOT/supabase/migrations/${previous_version}_chapas_2026_ma_vice_substituicao.sql"
rollback="$ROOT/supabase/rollback/${version}_backfill_sq_candidato_ondas_agosto.rollback.sql"
readback="$ROOT/supabase/readback/${version}_backfill_sq_candidato_ondas_agosto.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato do backfill de sq_candidato_2026 das ondas de agosto ausente" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: backfill de sq_candidato_2026 das ondas de agosto ja aplicado, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial do backfill de sq_candidato_2026 das ondas de agosto inesperado: $state" >&2
  exit 1
fi
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:backfill-sq-ondas-agosto-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'backfill sq ondas agosto: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'backfill sq ondas agosto: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: backfill de sq_candidato_2026 das ondas de agosto aplicado, ledger e readback concluidos"
