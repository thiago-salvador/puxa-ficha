#!/usr/bin/env bash
# Prova em PostgreSQL 17 descartavel da 20260903140000 (vice substituto da chapa
# de governador do MA). Fixture minima: a linha real de chapas_2026 mais duas
# linhas de controle que a troca NAO pode encostar, e coleta_log com os CHECKs
# reais de producao (medidos em 03/09/2026), os mesmos que a fixture de
# provar-chapas-hermano-pg17.sh carrega.
#
# Prova, em ordem: readback recusa o pre-estado; forward troca as sete colunas
# de uma linha; titular, alternativas_oficiais, vice_candidato_id e as duas
# linhas de controle ficam intactos; segunda aplicacao aborta; readback recusa
# adulteracao de linha vizinha, de coluna vizinha da propria linha alvo e de
# vinculo de ficha; rollback recusa migration posterior; rollback devolve a
# tabela byte a byte ao pre-estado; e, por fim, a migration aplica como no-op
# limpo quando a linha alvo nao existe, que e o caso do replay linear.
set -euo pipefail
...
PREVIOUS="20260903130000"
PREVIOUS_DIGEST="sha256:fixture-previous"
MIGRATION="supabase/migrations/${VERSION}_chapas_2026_ma_vice_substituicao.sql"
READBACK="supabase/readback/${VERSION}_chapas_2026_ma_vice_substituicao.readback.sql"
ROLLBACK="supabase/rollback/${VERSION}_chapas_2026_ma_vice_substituicao.rollback.sql"
ROLLBACK_READBACK="supabase/readback/${VERSION}_chapas_2026_ma_vice_substituicao.rollback.readback.sql"
...
# fixture aceitaria recibo que producao recusa.
q -q <<'SQL'
...
-- linhas de controle: uma vizinha da MESMA UF, com o SQ do vice substituto num
-- prefixo parecido, e uma chapa presidencial. Nenhuma das duas pode mudar, e o
-- digest agregado do readback e quem cobra. O titular_candidato_id da linha
-- alvo fica NULL porque `candidatos` nao existe nesta fixture; em producao ele
-- aponta para a ficha de REGINALDO LIMA e nao e tocado pela migration, coisa
-- que o digest do "resto da linha" prova de qualquer jeito.
...
VALUES
  ('2026:MA:reginaldo-lima-brauno', '6259', '2026-10-04', 'MA', 'Governador', '100001800701',
   'confirmada', 'novo_perfil_oficial', '#NE', '-3', '-3', 'PARTIDO ISOLADO',
   'PCB', '100002544073', '100002544074',
   'REGINALDO LIMA BRAUNO', 'REGINALDO LIMA', 'PCB',
   'BARTOLOMEU MOREIRA', 'BARTOLOMEU', 'PCB',
   'https://cdn.tse.jus.br/estatistica/sead/odsele/consulta_cand/consulta_cand_2026.zip',
   'eae2178d1d87c6f66c81ac5c6a56f10118a0bff373068135531315cec6f74a27', '2026-08-28T01:58:24.127Z'),
  ('2026:MA:controle-vizinho', '6259', '2026-10-04', 'MA', 'Governador', '100001800702',
   'confirmada', 'confirmado', '#NE', '-3', '-3', 'PARTIDO ISOLADO',
   'MISSÃO', '100002544076', '100002544075',
   'CONTROLE VIZINHO DA SILVA', 'CONTROLE', 'MISSÃO',
   'BARTOLOMEU DE CONTROLE', 'BARTOLOMEU CONTROLE', 'MISSÃO',
   'https://cdn.tse.jus.br/estatistica/sead/odsele/consulta_cand/consulta_cand_2026.zip',
   'eae2178d1d87c6f66c81ac5c6a56f10118a0bff373068135531315cec6f74a27', '2026-08-28T01:58:24.127Z'),
  ('2026:BR:controle-presidencial', '2026', '2026-10-04', NULL, 'Presidente', '100001800111',
   'confirmada', 'confirmado', '#NE', '-3', '-3', 'COLIGAÇÃO',
   'PARTIDO A / PARTIDO B', '100002511111', '100002511112',
...
# `2026:MA:controle-vizinho` carrega 'BARTOLOMEU CONTROLE' de proposito: um
# WHERE por LIKE, por prefixo de nome ou so por UF alcancaria essa linha, e o
# digest denunciaria.
vizinho_antes="$(q -Atq -c "SELECT vice_nome_urna FROM public.chapas_2026 WHERE chave='2026:MA:controle-vizinho'")"
[[ "$vizinho_antes" == "BARTOLOMEU CONTROLE" ]] || { echo "FAIL: fixture do vizinho errada" >&2; exit 1; }
...
alvo="$(q -Atq -c "SELECT vice_sq_candidato || '|' || vice_nome_urna || '|' || vice_nome_completo || '|' || vice_partido_sigla || '|' || tse_situacao_vice_codigo || '|' || fonte_sha256 || '|' || snapshot_em::text FROM public.chapas_2026 WHERE chave='2026:MA:reginaldo-lima-brauno'")"
esperado="100002554354|GATO FELIX|FELIX LIMA E SILVA|PCB|-3|b1b2613e246b85b7c3e002c3625232aac6abf5994a3639f1c834d6fda39b9217|2026-09-03 11:00:01.358+00"
[[ "$alvo" == "$esperado" ]] || { echo "FAIL: linha alvo apos forward = $alvo" >&2; exit 1; }

# O titular, o vinculo de ficha e as alternativas oficiais ficam de fora da
# troca. Sao as tres coisas que uma versao descuidada desta migration mexeria
# achando que "atualiza a chapa".
titular="$(q -Atq -c "SELECT titular_sq_candidato || '|' || titular_nome_urna || '|' || titular_partido_sigla || '|' || tse_situacao_titular_codigo || '|' || alternativas_oficiais::text || '|' || coalesce(vice_candidato_id::text, 'NULL') FROM public.chapas_2026 WHERE chave='2026:MA:reginaldo-lima-brauno'")"
[[ "$titular" == "100002544073|REGINALDO LIMA|PCB|-3|[]|NULL" ]] || { echo "FAIL: titular ou vinculo mudou: $titular" >&2; exit 1; }

intactas="$(q -Atq -c "SELECT count(*) FROM public.chapas_2026 WHERE chave <> '2026:MA:reginaldo-lima-brauno' AND vice_nome_urna IN ('BARTOLOMEU CONTROLE', 'VICE CONTROLE') AND fonte_sha256 = 'eae2178d1d87c6f66c81ac5c6a56f10118a0bff373068135531315cec6f74a27'")"
[[ "$intactas" == "2" ]] || { echo "FAIL: linhas de controle mudaram ($intactas de 2 intactas)" >&2; exit 1; }

recibo="$(q -Atq -c "SELECT escopo || '|' || resultado || '|' || volume || '|' || natureza || '|' || (detalhe::jsonb ->> 'outras_count') FROM public.coleta_log WHERE execucao='migration:$VERSION'")"
[[ "$recibo" == "territorio|encontrado|1|escrita|2" ]] || { echo "FAIL: recibo de pre-imagem = $recibo" >&2; exit 1; }
# A trilha do substituido: depois do UPDATE, BARTOLOMEU so existe no recibo.
preimagem="$(q -Atq -c "SELECT (detalhe::jsonb -> 'linhas' -> 0 ->> 'vice_sq_candidato') || '|' || (detalhe::jsonb -> 'linhas' -> 0 ->> 'vice_nome_urna') || '|' || (detalhe::jsonb -> 'linhas' -> 0 ->> 'vice_nome_completo') FROM public.coleta_log WHERE execucao='migration:$VERSION'")"
[[ "$preimagem" == "100002544074|BARTOLOMEU|BARTOLOMEU MOREIRA" ]] || { echo "FAIL: pre-imagem gravada = $preimagem" >&2; exit 1; }
sumiu="$(q -Atq -c "SELECT count(*) FROM public.chapas_2026 WHERE vice_sq_candidato='100002544074'")"
[[ "$sumiu" == "0" ]] || { echo "FAIL: o SQ da vice substituida sobrou em $sumiu linha(s)" >&2; exit 1; }
...
q -q -c "UPDATE public.chapas_2026 SET vice_nome_urna='ADULTERADO' WHERE chave='2026:MA:controle-vizinho'"
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: forward readback aceitou adulteracao de linha vizinha" >&2
  exit 1
fi
q -q -c "UPDATE public.chapas_2026 SET vice_nome_urna='BARTOLOMEU CONTROLE' WHERE chave='2026:MA:controle-vizinho'"
q -q < "$READBACK"

# Adulteracao 2: coluna vizinha DA PROPRIA linha alvo. O titular e o que a
# migration promete nao tocar, e a linha alvo fica fora do digest agregado do
# recibo, entao so uma asseveracao explicita do readback pega esse estrago.
q -q -c "UPDATE public.chapas_2026 SET titular_nome_urna='REGINALDO ADULTERADO' WHERE chave='2026:MA:reginaldo-lima-brauno'"
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: forward readback aceitou adulteracao do titular" >&2
  exit 1
fi
q -q -c "UPDATE public.chapas_2026 SET titular_nome_urna='REGINALDO LIMA' WHERE chave='2026:MA:reginaldo-lima-brauno'"
q -q < "$READBACK"

# Adulteracao 3: vinculo de ficha do vice. O readback cobra vice_candidato_id
# NULL em voz alta, porque vice de chapa estadual nao tem ficha publica aqui.
q -q -c "UPDATE public.chapas_2026 SET vice_candidato_id=gen_random_uuid() WHERE chave='2026:MA:reginaldo-lima-brauno'"
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: forward readback aceitou vinculo de ficha no vice" >&2
  exit 1
fi
q -q -c "UPDATE public.chapas_2026 SET vice_candidato_id=NULL WHERE chave='2026:MA:reginaldo-lima-brauno'"
q -q < "$READBACK"

# Adulteracao 4: proveniencia revertida a mao. O carimbo de pacote e parte do
# que a migration afirma, entao desfaze-lo por fora tem que reprovar.
q -q -c "UPDATE public.chapas_2026 SET fonte_sha256='eae2178d1d87c6f66c81ac5c6a56f10118a0bff373068135531315cec6f74a27' WHERE chave='2026:MA:reginaldo-lima-brauno'"
if q -q < "$READBACK" >/dev/null 2>&1; then
  echo "FAIL: forward readback aceitou fonte_sha256 revertido" >&2
  exit 1
fi
q -q -c "UPDATE public.chapas_2026 SET fonte_sha256='b1b2613e246b85b7c3e002c3625232aac6abf5994a3639f1c834d6fda39b9217' WHERE chave='2026:MA:reginaldo-lima-brauno'"
q -q < "$READBACK"
...
q -q < "$MIGRATION"
noop="$(q -Atq -c "SELECT resultado || '|' || volume FROM public.coleta_log WHERE execucao='migration:$VERSION'")"
[[ "$noop" == "vazio_confirmado|0" ]] || { echo "FAIL: no-op de replay gravou recibo $noop" >&2; exit 1; }

echo "PASS: 20260903140000 provada em PostgreSQL 17 com 24 checagens (forward das sete colunas, titular e vinculo intactos, linhas de controle intactas, recibo com pre-imagem, SQ substituido ausente da tabela, reaplicacao recusada, quatro adulteracoes, migration posterior, rollback byte a byte, segundo rollback recusado, ledger e no-op de replay)"
