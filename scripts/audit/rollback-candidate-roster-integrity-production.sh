...
print(f"DELETE FROM supabase_migrations.schema_migrations WHERE (version={lit(data_version)} AND idempotency_key={lit(data_digest)}) OR (version={lit(schema_version)} AND idempotency_key={lit(schema_digest)});")
