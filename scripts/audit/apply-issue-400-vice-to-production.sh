#!/usr/bin/env bash
# Aplica a migration de dado da issue #400 (vice TO) (espelhada pelo cron #406):
#   20260921220000  vice CAPITAO OSMAR (TO) Deferido no detalhe oficial do TSE
# com predecessor, hash, lock, ledger e readback fechados para o projeto de
# producao do Puxa Ficha. Clone de apply-issues-378-383-production.sh reduzido a
# uma migration.
#
...
#
#   scripts/audit/apply-issue-400-vice-to-production.sh dry-run   # ensaio, nao grava
#   scripts/audit/apply-issue-400-vice-to-production.sh apply     # grava
set -euo pipefail
...
base_version=20260921200000
base_migration="$ROOT/supabase/migrations/${base_version}_issue_400_policial_edjane_indeferido_terminal.sql"
...
versions=(20260921220000)
names=(issue_400_vice_to_capitao_osmar_deferido)
...
# readbacks rodam.
if [[ "$c0" == "1" && "$k0" == "$digest0" ]]; then
  [[ "$topo" == "${versions[0]}" ]] || { echo "FAIL: a migration esta no ledger mas o topo e $topo" >&2; exit 1; }
  rodar_readbacks
  echo "PASS: issue #400 (vice TO) ja aplicada, ledger e readback conferem"
  exit 0
fi

# O predecessor 20260921200000 foi gravado por apply-issue-400-production.sh,
# que sempre escreve digest; aqui a exigencia e estrita.
...
for i in range(0, len(resto), 5):
    ...
    print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'issue-400-vice-to: ledger final divergiu em {version}'; END IF; END $ledger$;")
...
if [[ "$modo" == "dry-run" ]]; then
  ...
  echo "PASS: dry-run da issue #400 (vice TO) rodou migration, ledger e readback e desfez tudo"
  exit 0
fi
...
gerar_sql ROLLBACK "${args[@]}" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -
echo "PASS: ensaio pre-apply da issue #400 (vice TO) conferido; gravando"

gerar_sql COMMIT "${args[@]}" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -

rodar_readbacks
echo "PASS: issue #400 (vice TO) aplicada, ledger e readbacks concluidos"
