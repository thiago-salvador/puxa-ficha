#!/usr/bin/env bash
# Reverte 20260903220000 pela pre-imagem gravada em coleta_log. ATENCAO: reverter
# REPUBLICA trajetoria e dinheiro de campanha de outra pessoa na ficha.
set -euo pipefail
...
rollback_readback="$ROOT/supabase/readback/${version}_despublicar_alvaro_dias_rn_homonimo.rollback.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$rollback_readback" ]] || {
  echo "FAIL: artefato de rollback da despublicacao do homonimo de alvaro-dias-rn ausente" >&2
  exit 2
}
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'rollback alvaro-dias-rn homonimo: ledger divergiu sob lock'; END IF; END $ledger$;")
print(body, end="" if body.endswith("\n") else "\n")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'rollback alvaro-dias-rn homonimo: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY
...
echo "PASS: rollback da despublicacao do homonimo concluido, pre-imagem conferida"
