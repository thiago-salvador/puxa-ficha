...
DELTAS_ESPERADOS = {
    ...
    (
        ...
        "OR (cargo_disputado <> ALL (ARRAY['Presidente'::text, 'Governador'::text, 'Senador'::text])) OR "
    ...
    (
        ...
        '"pendente de julgamento" entrou em 16/09/2026 (issue #340), DS_SITUACAO_JULGAMENTO codigo 17 de '
        'consulta_cand_complementar, distinto de "aguardando julgamento" (codigo 8) e de proposito fora de '
        "SITUACAO_JULGAMENTO_PUBLICADO: nao e deferimento nem indeferimento.';",
    ),
    (
        ">",
    ...
    (
        ...
        "'deferido com recurso'::text, 'indeferido'::text, 'indeferido com recurso'::text, 'pendente de julgamento'::text])))",
    ),
}
