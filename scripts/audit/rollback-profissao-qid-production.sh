#!/usr/bin/env bash
# Reverte somente a migration 20260830120000, recusando qualquer migration ou
# curadoria posterior e comprovando o estado anterior em readback separado.
set -euo pipefail
...
previous_version=20260829100100
migration="$ROOT/supabase/migrations/${version}_backfill_profissao_declarada_qid_wikidata.sql"
rollback="$ROOT/supabase/rollback/${version}_backfill_profissao_declarada_qid_wikidata.rollback.sql"
rollback_readback="$ROOT/supabase/readback/${version}_backfill_profissao_declarada_qid_wikidata.rollback.readback.sql"
previous="$ROOT/supabase/migrations/${previous_version}_backfill_projetos_lei_camara_ronaldo_caiado.sql"
[[ -f "$migration" && -f "$rollback" && -f "$rollback_readback" && -f "$previous" ]] || {
  echo "FAIL: artefato de rollback ou predecessor ausente" >&2
  exit 2
}
...
[[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]] || {
  echo "FAIL: rollback de profissao QID exige ledger exato no topo: $state" >&2
  exit 1
}
...
if len(begins) != 1 or len(commits) != 1 or begins[0].end() >= commits[0].start():
    raise SystemExit("rollback de profissao QID deve ter BEGIN/COMMIT externos unicos")
body = text[begins[0].end():commits[0].start()]
def lit(value): return "'" + value.replace("'", "''") + "'"

print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:profissao-qid-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'rollback profissao QID: ledger divergiu sob lock'; END IF; END $ledger$;")
print(body, end="" if body.endswith("\n") else "\n")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'rollback profissao QID: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY
...
echo "PASS: rollback de profissao QID concluído, predecessor e estado anterior conferem"
