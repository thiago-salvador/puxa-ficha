...
[[ -f "$migration" && -f "$readback" && -f "$prior" && -f "$receipt" ]] || exit 2
...
if [[ "$top" == "$version" && "$prior_count" == 1 && "$prior_key" == "$prior_digest" && "$target_count" == 1 && "$target_key" == "$digest" ]]; then
...
[[ "$top" == "$previous" && "$prior_count" == 1 && "$prior_key" == "$prior_digest" && "$target_count" == 0 ]] || {
  echo "FAIL: ledger inicial divergente: $ledger" >&2; exit 1;
}

python3 - "$PF_EXPECTED_SHA" "$mode" "$version" "$previous" "$digest" "$prior_digest" "$migration" "$readback" <<'PY' | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -
import base64, pathlib, re, sys
sha, mode, version, previous, digest, prior_digest, migration, readback = sys.argv[1:]
...
print(f"{lit(version)}, ARRAY[convert_from(decode({lit(encoded)},'base64'),'UTF8')], 'quarentena_gastos_parlamentares_57_linhas', {lit('Thiago Salvador <contato.thiagosalvador@gmail.com> via github-actions:' + sha)}, {lit(digest)});")
readback_body = re.sub(r'(?im)^\s*(?:BEGIN READ ONLY|COMMIT);\s*$', '', readback_text)
print(readback_body)
print('RESET ROLE;')
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'gastos: ledger final divergiu'; END IF; END $ledger$;")
