#!/usr/bin/env bash
# Applies the verified-history RPC status domain migration after its predecessor.
...
previous_version=20260924003000
previous_migration="$ROOT/supabase/migrations/${previous_version}_senado_exercicio_reaberto.sql"
migration="$ROOT/supabase/migrations/${version}_verified_history_situacao_dominio.sql"
rollback="$ROOT/supabase/rollback/${version}_verified_history_situacao_dominio.rollback.sql"
readback="$ROOT/supabase/readback/${version}_verified_history_situacao_dominio.readback.sql"
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'verified history situacao: final ledger diverged'; END IF; END $ledger$;")
print("COMMIT;")
PY
