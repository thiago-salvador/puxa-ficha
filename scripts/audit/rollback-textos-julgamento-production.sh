#!/usr/bin/env bash
# Restaura a pre-imagem textual exata sem alterar outros campos.
# Convenção operacional da 20260904220000; sem db push ou migration automática.
set -euo pipefail
...
previous_version=20260904220000
migration="$ROOT/supabase/migrations/${version}_corrigir_textos_julgamento.sql"
previous_migration="$ROOT/supabase/migrations/${previous_version}_corrigir_profissao_alvaro_dias_rn.sql"
rollback="$ROOT/supabase/rollback/${version}_corrigir_textos_julgamento.rollback.sql"
rollback_readback="$ROOT/supabase/readback/${version}_corrigir_textos_julgamento.rollback.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$rollback_readback" ]] || {
  echo "FAIL: artefato de rollback da correcao dos textos de julgamento ausente" >&2
  exit 2
}
...
python3 - "$version" "$previous_version" "$digest" "$previous_digest" "$rollback" "$rollback_readback" <<'PY' | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -
import pathlib, re, sys

version, previous, digest, previous_digest, rollback_path, readback_path = sys.argv[1:]
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:textos-julgamento-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'rollback textos-julgamento: ledger divergiu sob lock'; END IF; END $ledger$;")
print(body, end="" if body.endswith("\n") else "\n")
print(pathlib.Path(readback_path).read_text(encoding="utf-8"))
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'rollback textos-julgamento: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY
...
echo "PASS: rollback da correcao dos textos de julgamento concluido, pre-imagem conferida"
