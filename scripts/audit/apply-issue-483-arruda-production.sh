#!/usr/bin/env bash
# Aplica a migration de dado da issue #483:
#   20260924180000  jose-roberto-arruda Indeferido (sem recurso) no detalhe oficial do TSE
# com predecessor, hash, lock, ledger e readback fechados para o projeto de
# producao do Puxa Ficha. Clone de apply-issue-433-dimas-cassimiro-production.sh.
#
...
#
#   scripts/audit/apply-issue-483-arruda-production.sh dry-run   # ensaio, nao grava
#   scripts/audit/apply-issue-483-arruda-production.sh apply     # grava
set -euo pipefail
...
base_version=20260924120000
base_migration="$ROOT/supabase/migrations/${base_version}_verified_history_situacao_dominio.sql"
...
versions=(20260924180000)
names=(issue_483_arruda_indeferido)
...
if [[ "$c0" == "1" && "$k0" == "$digest0" ]]; then
  ...
  rodar_readbacks
  echo "PASS: issue #483 ja aplicada, ledger e readback conferem"
  exit 0
fi

# O predecessor 20260924120000 foi gravado por outro apply, que sempre
...
for i in range(0, len(resto), 5):
    ...
    print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'issue-483: ledger final divergiu em {version}'; END IF; END $ledger$;")
...
if [[ "$modo" == "dry-run" ]]; then
  ...
  echo "PASS: dry-run da issue #483 rodou migration, ledger e readback e desfez tudo"
  exit 0
fi
...
gerar_sql ROLLBACK "${args[@]}" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -
echo "PASS: ensaio pre-apply da issue #483 conferido; gravando"

gerar_sql COMMIT "${args[@]}" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -f -

rodar_readbacks
echo "PASS: issue #483 aplicada, ledger e readbacks concluidos"
