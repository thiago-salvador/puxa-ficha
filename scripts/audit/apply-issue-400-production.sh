#!/usr/bin/env bash
# Aplica a migration de dado da issue #400 (espelhada pelo cron #406):
#   20260921200000  policial-edjane terminal no detalhe oficial do TSE
...
#
#   scripts/audit/apply-issue-400-production.sh dry-run   # ensaio, nao grava
#   scripts/audit/apply-issue-400-production.sh apply     # grava
set -euo pipefail
...
base_version=20260918120100
base_migration="$ROOT/supabase/migrations/${base_version}_issue_383_gustavo_henrique_indeferido_terminal.sql"
...
versions=(20260921200000)
names=(issue_400_policial_edjane_indeferido_terminal)
...
if [[ "$c0" == "1" && "$k0" == "$digest0" ]]; then
  ...
  rodar_readbacks
  echo "PASS: issue #400 ja aplicada, ledger e readback conferem"
  exit 0
fi

# O predecessor 20260918120100 foi gravado pelo apply das issues #378/#383,
...
for i in range(0, len(resto), 5):
    ...
    print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'issue-400: ledger final divergiu em {version}'; END IF; END $ledger$;")
...
rodar_readbacks
echo "PASS: issue #400 aplicada, ledger e readbacks concluidos"
