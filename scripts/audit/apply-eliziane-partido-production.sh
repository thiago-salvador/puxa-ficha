#!/usr/bin/env bash
# Applies the Eliziane party correction migration after its fixed predecessor.
set -euo pipefail
...
readback="$ROOT/supabase/readback/${version}_eliziane_partido_tse_2026.readback.sql"
[[ -f "$previous_migration" && -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: required migration artifacts are missing" >&2
  exit 2
}
[[ "sha256:$(shasum -a 256 "$previous_migration" | cut -d' ' -f1)" == "$previous_digest" ]] || {
  echo "FAIL: fixed predecessor hash differs" >&2
  exit 2
}
...
DO \$preflight\$ BEGIN
  IF (SELECT count(*) FROM public.candidatos c
      WHERE c.id = 'b8e8b3d1-1e2e-482f-b0dd-dbf927c5c681'
        ...
        AND c.numero_urna = '133' AND c.partido_sigla = 'PSD'
        AND c.partido_atual = 'PSD' AND c.publicavel IS TRUE
        AND c.status <> 'removido') <> 1
     OR EXISTS (SELECT 1 FROM public.coleta_log WHERE execucao = 'migration:20260923175946')
     OR EXISTS (SELECT 1 FROM public.identidade_timeline_quarentena_snapshot
                 WHERE migration_version = '20260923175946') THEN
    RAISE EXCEPTION 'eliziane partido: preimagem ou recibo divergiu';
  END IF;
END \$preflight\$;"
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'eliziane partido: final ledger diverged'; END IF; END $ledger$;")
print("COMMIT;")
PY
