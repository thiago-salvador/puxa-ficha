...
dry_run="0"
existing_override=""
existing_fingerprint_override=""
for argument in "${@:5}"; do
  case "$argument" in
    --dry-run) dry_run="1" ;;
    --existing=*) existing_override="${argument#--existing=}" ;;
    --existing-fingerprint=*) existing_fingerprint_override="${argument#--existing-fingerprint=}" ;;
...
fingerprint="$({
  ...
  if [[ -s "$summary_path" ]]; then
    sed -E '/^- Gerada em:/d' "$summary_path"
  else
    printf '%s\n' 'auditoria sem resumo'
  fi
} | {
...
fingerprint_marker="<!-- data-freshness-fingerprint:${fingerprint} -->"
...
existing_fingerprint="$existing_fingerprint_override"
if [[ "$dry_run" != "1" && -n "$existing" ]]; then
  existing_fingerprint="$(
    gh api "repos/${repo}/issues/${existing}" --jq '.body // ""' |
      sed -nE 's/.*data-freshness-fingerprint:([a-f0-9]{64}).*/\1/p' |
      sed -n '1p'
  )"
fi
...
if [[ "$dry_run" == "1" ]]; then
  if [[ -n "$existing" ]]; then
    if [[ "$existing_fingerprint" == "$fingerprint" ]]; then
      printf 'ação: atualizar issue #%s sem novo comentário; ocorrência não mudou\n' "$existing"
    else
      printf 'ação: atualizar issue #%s e comentar alteração da ocorrência\n' "$existing"
    fi
  else
...
if [[ -n "$existing" ]]; then
  jq -n \
    ...
    gh api --method PATCH "repos/${repo}/issues/${existing}" --input - >/dev/null
  if [[ "$existing_fingerprint" == "$fingerprint" ]]; then
    printf 'Incidente #%s atualizado sem comentário duplicado.\n' "$existing"
  else
    jq -n --arg body "🚨 A ocorrência mudou. Evidências e recomendações atualizadas: ${run_url}" '{body:$body}' |
      gh api --method POST "repos/${repo}/issues/${existing}/comments" --input - >/dev/null
