#!/usr/bin/env bash
# Aplica 20260916140000 (issue #340: seis situacoes de candidatos,
# despublicacao formal de carlos-jararaca, tres chapas de RN/SE substituidas)
...
readback="$ROOT/supabase/readback/${version}_reconciliar_situacoes_e_chapas_16092026.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato da reconciliacao de situacoes e chapas (issue #340) ausente" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: reconciliacao de situacoes e chapas (issue #340) ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial da reconciliacao de situacoes e chapas (issue #340) inesperado: $state" >&2
  exit 1
fi
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'issue-340 situacoes e chapas: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: reconciliacao de situacoes e chapas (issue #340) aplicada, ledger e readback concluidos"
