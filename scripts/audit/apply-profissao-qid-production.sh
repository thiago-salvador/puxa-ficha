#!/usr/bin/env bash
# Aplica somente a migration 20260830120000, com predecessor, hash, lock,
# ledger e readback fechados para o projeto de produção do Puxa Ficha.
set -euo pipefail
...
readback="$ROOT/supabase/readback/${version}_backfill_profissao_declarada_qid_wikidata.readback.sql"
...
[[ -f "$migration" && -f "$rollback" && -f "$readback" && -f "$previous" ]] || {
  echo "FAIL: artefato de profissao QID ou predecessor ausente" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: profissao QID ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial de profissao QID inesperado: $state" >&2
  exit 1
fi
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'profissao QID: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: profissao QID aplicada, ledger e readback concluídos"
