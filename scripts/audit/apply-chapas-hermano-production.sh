#!/usr/bin/env bash
# Aplica a 20260903130000 (nome de urna do vice de RN) numa unica transacao, com
# predecessor, hash, lock, ledger e readback fechados, no projeto de producao do
# Puxa Ficha.
set -euo pipefail
...
previous_version=20260903120000
migration="$ROOT/supabase/migrations/${version}_chapas_2026_hermano_nome_urna.sql"
previous_migration="$ROOT/supabase/migrations/${previous_version}_drop_indices_sem_uso.sql"
rollback="$ROOT/supabase/rollback/${version}_chapas_2026_hermano_nome_urna.rollback.sql"
readback="$ROOT/supabase/readback/${version}_chapas_2026_hermano_nome_urna.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato da correcao de chapas_2026 ausente" >&2
  exit 2
}
# O predecessor e a 20260903120000, que chega por outra PR. O digest dela e
# CALCULADO do arquivo, nunca copiado para dentro deste script: um literal aqui
# seria uma segunda copia do mesmo hash, livre para divergir em silencio da
# migration real. Arquivo ausente aborta antes de qualquer conexao.
[[ -f "$previous_migration" ]] || {
  echo "FAIL: predecessor ${previous_version} ausente em supabase/migrations/; esta correcao so aplica depois dele" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: correcao do nome de urna do vice de RN ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial da correcao de chapas_2026 inesperado: $state" >&2
  exit 1
fi
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:chapas-hermano-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'chapas hermano: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'chapas hermano: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: correcao do nome de urna do vice de RN aplicada, ledger e readback concluidos"
