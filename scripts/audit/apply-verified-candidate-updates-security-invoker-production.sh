#!/usr/bin/env bash
# Aplica somente a migration 20260916120000, com predecessor, hash, lock,
...
previous_digest=sha256:845d75c8c65f267537f43eeffba7a81bb38fa3970a5fd65b627b22f63f7dd697
migration="$ROOT/supabase/migrations/${version}_verified_candidate_updates_security_invoker.sql"
rollback="$ROOT/supabase/rollback/${version}_verified_candidate_updates_security_invoker.rollback.sql"
readback="$ROOT/supabase/readback/${version}_verified_candidate_updates_security_invoker.readback.sql"
[[ -f "$migration" && -f "$rollback" && -f "$readback" ]] || {
  echo "FAIL: artefato da view invoker do historico verificado ausente" >&2
  exit 2
}
...
if [[ "$ledger_top" == "$version" && "$version_count" == "1" && "$version_key" == "$digest" && "$previous_count" == "1" && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: view invoker do historico verificado ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$previous_version" || "$previous_count" != "1" || "$previous_key" != "$previous_digest" || "$version_count" != "0" ]]; then
  echo "FAIL: ledger inicial da view invoker do historico verificado inesperado: $state" >&2
  exit 1
fi
...
print("BEGIN;")
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:verified-candidate-updates-security-invoker-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'view invoker do historico verificado: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'view invoker do historico verificado: ledger final divergiu'; END IF; END $ledger$;")
print("COMMIT;")
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: view invoker do historico verificado aplicada, ledger e readback concluidos"
