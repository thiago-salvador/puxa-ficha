#!/usr/bin/env bash
# Aplica uma das duas migrations da Frente 3 com SHA, predecessor, ledger e readback fechados.
set -euo pipefail
...
case "${1:-}" in
  roster)
    ...
    predecessor=20260923001140
    name=candidatos_roster_2026
    predecessor_name=despublicar_48_claims_fontes
    ;;
  colinha)
    version=20260923130000
    predecessor=20260923120000
    name=analytics_colinha_share
    predecessor_name=candidatos_roster_2026
    ;;
...
previous_migration="$ROOT/supabase/migrations/${predecessor}_${predecessor_name}.sql"
rollback="$ROOT/supabase/rollback/${version}_${name}.rollback.sql"
readback="$ROOT/supabase/readback/${version}_${name}.readback.sql"
for file in "$migration" "$previous_migration" "$rollback" "$readback"; do
  [[ -f "$file" ]] || { echo "FAIL: artefato ausente: ${file#"$ROOT"/}" >&2; exit 2; }
done
...
state="$(PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  ...
  "select coalesce(max(version),'') || '|' || count(*) filter(where version='$predecessor') || '|' || coalesce(max(idempotency_key) filter(where version='$predecessor'),'') || '|' || count(*) filter(where version='$version') || '|' || coalesce(max(idempotency_key) filter(where version='$version'),'') from supabase_migrations.schema_migrations")"
...
if [[ "$ledger_top" == "$version" && "$version_count" == 1 && "$version_key" == "$digest" && "$previous_count" == 1 && "$previous_key" == "$previous_digest" ]]; then
  ...
  echo "PASS: ${name} ja aplicada, ledger e readback conferem"
  exit 0
fi
if [[ "$ledger_top" != "$predecessor" || "$previous_count" != 1 || "$previous_key" != "$previous_digest" || "$version_count" != 0 ]]; then
  echo "FAIL: ledger inicial de ${name} inesperado: $state" >&2
  exit 1
fi

python3 - "$PF_EXPECTED_SHA" "$version" "$predecessor" "$digest" "$previous_digest" "$migration" "$rollback" "$readback" <<'PY' | \
...
begins = list(re.finditer(r'(?im)^\s*BEGIN;\s*$', source))
commits = list(re.finditer(r'(?im)^\s*COMMIT;\s*$', source))
if len(begins) != 1 or len(commits) != 1 or begins[0].end() >= commits[0].start():
    raise SystemExit('FAIL: migration deve ter BEGIN e COMMIT externos unicos')
body = source[begins[0].end():commits[0].start()]
rollback = pathlib.Path(rollback_path).read_bytes()
readback = pathlib.Path(readback_path).read_text()
...
print('BEGIN;')
print("SELECT pg_advisory_xact_lock(hashtextextended('puxa-ficha:frente3-migrations-production', 0));")
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(previous)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(previous)} AND idempotency_key={lit(previous_digest)}) <> 1 OR EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version={lit(version)}) THEN RAISE EXCEPTION 'frente3: ledger divergiu sob lock'; END IF; END $ledger$;")
...
print(f"DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) <> {lit(version)} OR (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version={lit(version)} AND idempotency_key={lit(digest)}) <> 1 THEN RAISE EXCEPTION 'frente3: ledger final divergiu'; END IF; END $ledger$;")
print('COMMIT;')
PY

PGOPTIONS='-c default_transaction_read_only=on -c statement_timeout=300000 -c lock_timeout=5000' \
  psql -X -v ON_ERROR_STOP=1 -f "$readback"
echo "PASS: ${name} aplicada, ledger e readback concluidos"
