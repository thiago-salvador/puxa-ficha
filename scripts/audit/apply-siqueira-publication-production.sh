#!/usr/bin/env bash
# Guardas canônicas da PR262: SHA exato, main remoto, checkout limpo e projeto/TLS.
# Nenhuma execução remota é autorizada pela existência deste driver.
set -euo pipefail
...
mode="${1:-dry-run}"
case "$mode" in apply|dry-run|verify|backup) ;; *) echo "FAIL: modo inválido" >&2; exit 2 ;; esac
if [[ "$mode" == "backup" ]]; then
  bash scripts/audit/backup-closeout-production.sh "${2:?diretorio de backup obrigatorio}"
  exit 0
fi
node --import tsx scripts/audit/apply-siqueira-publication.ts "$mode" "$PF_EXPECTED_SHA" | \
  PGOPTIONS='-c statement_timeout=300000 -c lock_timeout=5000' psql -X -v ON_ERROR_STOP=1 -v VERBOSITY=terse -f -
if [[ "$mode" == "apply" ]]; then
  node --import tsx scripts/audit/apply-siqueira-publication.ts verify "$PF_EXPECTED_SHA" | \
...
echo "PASS: siqueira-publication mode=$mode"
