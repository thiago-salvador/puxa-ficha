...
export const VICE_STATUS_PREDECESSOR = "20260908160000"
...
export function renderViceStatusTransaction(
  ...
  const predecessorDigest = digest(read(`supabase/migrations/${VICE_STATUS_PREDECESSOR}_verified_candidate_updates.sql`))
  const files = VICE_STATUS_MIGRATIONS.map((file) => {
  ...
  const top = applied ? files.at(-1)!.version : VICE_STATUS_PREDECESSOR
  const expected = [
    [VICE_STATUS_PREDECESSOR, predecessorDigest],
  ...
  const guard = `DO $ledger$ BEGIN IF (SELECT max(version) FROM supabase_migrations.schema_migrations) IS DISTINCT FROM ${lit(top)} OR ${checks.join(" OR ")} THEN RAISE EXCEPTION 'vice-status: ledger or digest drift'; END IF; END $ledger$;`
  ...
  for (const file of files) {
    ...
    sql.push(`DO $proof$ BEGIN IF (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version=${lit(file.version)} AND idempotency_key=${lit(digest(file.raw))} AND name=${lit(file.name)} AND statements=ARRAY[${encoded(file.raw)}] AND rollback=ARRAY[${encoded(file.rollback)}] AND created_by=${lit(AUTHOR + sha)}) <> 1 THEN RAISE EXCEPTION 'vice-status: stored migration provenance mismatch'; END IF; END $proof$;`)
  }
  sql.push(`DO $authors$ BEGIN IF (SELECT count(DISTINCT created_by) FROM supabase_migrations.schema_migrations WHERE version IN (${files.map((file) => lit(file.version)).join(",")})) <> 1 THEN RAISE EXCEPTION 'vice-status: inconsistent applied SHA'; END IF; END $authors$;`)
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  process.stdout.write(renderViceStatusTransaction(mode as Parameters<typeof renderViceStatusTransaction>[0], sha))
}
