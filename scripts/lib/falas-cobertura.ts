...
export function paginaCobertura(coverage: ReturnType<typeof medirCobertura>, catalog: CatalogoFalas): string {
  ...
  const rows = [...coverage.candidates].sort((a,b) => (a.uf ?? "BR").localeCompare(b.uf ?? "BR") || a.name.localeCompare(b.name, "pt-BR")).map((candidate) => {
    ...
    const detail = quotes.length ? quotes.map((q) => `<blockquote>“${escape(q.quote_text)}”<footer>${escape(q.event_type === "declaracao" ? "Declaração em campanha" : q.event_type)} · ${q.occurred_on ? date(q.occurred_on) : q.occurred_between!.from === q.occurred_between!.to ? date(q.occurred_between!.from) : `Entre ${date(q.occurred_between!.from)} e ${date(q.occurred_between!.to)} (dia exato não informado)`}${periodoDentro(q, new Date(coverage.observed_at), naJanela) ? "" : " · Histórico"} · <a href="${escape(q.article_url)}" target="_blank" rel="noopener noreferrer">${escape(q.publisher)}</a>${q.transcription ? ` · Transcrição automática, pode conter erros. <a href="${escape(q.transcription.media_url + (q.transcription.media_url.includes("youtube.com/watch?") ? `&t=${Math.floor(q.transcription.start_seconds)}s` : `#t=${Math.floor(q.transcription.start_seconds)}`))}" target="_blank" rel="noopener noreferrer">Conferir áudio no trecho</a>` : ""}</footer></blockquote>`).join("")
    ...
    return `<article data-covered="${quotes.length > 0}" data-search="${escape(`${candidate.name} ${candidate.uf ?? "BR"} ${candidate.office}`.toLocaleLowerCase("pt-BR"))}"><h2>${escape(candidate.name)} <span>${escape(candidate.office)} · ${escape(candidate.uf ?? "BR")}</span></h2>${detail}${pendingReasons}<details><summary>Consultas e URLs registradas (${candidate.queries.length})</summary><ul>${candidate.queries.map((q) => `<li>${escape(q)}</li>`).join("")}</ul><ul>${candidate.urls.map((url) => `<li><a href="${escape(url)}" target="_blank" rel="noopener noreferrer">${escape(url)}</a></li>`).join("")}</ul></details></article>`
  }).join("")
