...
async function main() {
  ...
  for (let index = 0; index < args.length; index += 2) {
    if (!["--out", "--source"].includes(args[index]) || !args[index + 1] || args[index + 1].startsWith("--")) throw new Error("opção de descoberta inválida")
  ...
  const payload = {
    ...
    status: observations.some((observation) => observation.status !== "observed") ? "source_failure" : pending.length ? "new_urls_pending_validation" : "no_new_urls_in_consulted_listings",
