    ...
    <section data-colinha-screen="true" className="mx-auto max-w-7xl px-5 py-10 md:px-12 lg:py-16">
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="@container">
          ...
          <select id="colinha-uf" value={state.uf ?? ""} onChange={(event) => { const uf = event.target.value || null; updateState({ ...EMPTY_STATE, uf }); setResults([]); setIssues({}); setSnapshot(null); setSnapshotChecked(false); setUnavailable(false) }} className="mt-2 min-h-12 w-full rounded-lg border border-border bg-card px-3 text-base font-semibold text-foreground outline-none focus-visible:ring-2 focus-visible:ring-foreground/30"><option value="">Escolha uma UF</option>{UFS.map((uf) => <option key={uf} value={uf}>{uf}</option>)}</select>
          {state.uf && <div className="mt-7 grid gap-3 @xl:grid-cols-2">{SLOT_ORDER.map((slot, index) => { const choice = choices[slot]; const issue = issues[slot]; return <article key={slot} className={`rounded-xl border p-4 ${issue ? "border-amber-300" : "border-border"}`}><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">{String(index + 1).padStart(2, "0")}</p><h3 className="mt-1 font-bold text-foreground">{SLOT_LABELS[slot]}</h3></div>{choice && <Check aria-label="Escolha preenchida" className="size-5 text-emerald-600" />}</div>{choice ? <div className="mt-4 flex flex-wrap items-center gap-3"><CandidatePhoto src={choice.foto_path} alt={imageAlt(choice)} name={choice.nome_urna} width={48} height={60} sizes="48px" className="size-12 shrink-0 rounded-md object-cover" initialsClassName="text-xs" /><div className="min-w-[9rem] flex-1"><p className="truncate font-bold text-foreground">{choice.nome_urna}</p><p className="text-sm text-muted-foreground">{choice.numero_urna} · {choice.partido_sigla}</p><p className="text-xs text-muted-foreground">{status(choice)}</p>{renderSummary(choice)}</div><div className="flex shrink-0 items-center">{choice.slug && <Link href={`/candidato/${choice.slug}`} target="_blank" className="p-2 text-muted-foreground" aria-label={`Abrir ficha de ${choice.nome_urna}`}><ExternalLink aria-hidden="true" className="size-4" /></Link>}<button type="button" onClick={() => clearSlot(slot)} className="min-h-11 px-2 text-xs font-bold text-muted-foreground underline underline-offset-2">Trocar</button></div></div> : <><button type="button" onClick={() => { setActiveSlot(slot); setResults([]) }} aria-label={`Escolher candidato para ${SLOT_LABELS[slot]}`} className="mt-4 min-h-11 w-full rounded-lg border border-dashed border-border text-sm font-bold text-muted-foreground hover:border-foreground hover:text-foreground">Escolher candidato</button>{issue && <p className="mt-3 flex items-start gap-2 text-xs font-semibold text-amber-800"><AlertTriangle aria-hidden="true" className="mt-0.5 size-4 shrink-0" />{issue}</p>}</>}</article> })}</div>}
