/**
 * Vocabulario fechado de `candidatos.situacao_candidatura`.
 *
 * Ate 16/08/2026 o campo era TEXT livre e o banco guardava onze grafias para
 * tres sentidos, porque cada rodada de ingestao inventou a propria redacao. A
 ...
 * mesmo dominio: mudou aqui, muda no CHECK na MESMA PR, e vice-versa. O teste
 * `tests/situacao-candidatura-dominio.test.ts` compara os dois por parse, para
 * a divergencia falhar no CI em vez de aparecer no ar.
 *
 ...
 * distincao inventada. `deferido` e `indeferido` entram no dia em que o TSE
 * publicar julgamento, numa PR deliberada.
 *
 * `null` e valor legitimo e nao esta nesta lista de proposito: significa
 * ausencia de informacao, o CHECK deixa passar por construcao, e nenhuma ficha
 * publicavel pode ficar assim (regra em `published-consistency.ts`).
 *
