...
/**
 * Evidências públicas ligadas aos temas do programa do candidato. Falha de
 * leitura (view ausente antes da migration, rede) devolve lista vazia: a seção
 * mostra o estado vazio e a ficha não quebra.
 */
export async function getCompromissoEvidenciasPublicas(candidatoId: string, programaChave: string): Promise<CompromissoEvidenciaPublica[]> {
  try {
    const db = createServiceRoleSupabaseClient({ cacheMode: "isr" })
    ...
    const [votos, projetos, posicoes, pontos] = await Promise.all([
      ...
      porIds<{ id: string; titulo: string; fontes: Array<{ url?: string }> | null; data_referencia: string | null; visivel: boolean | null }>(
    ...
    for (const linha of linhas) {
      ...
      if (linha.tipo_evidencia === "votacao_chave") {
        ...
        saida.push({ ...base, referencia: "", texto: ponto.titulo, data: ponto.data_referencia, url: urlSeguraDeFonte(ponto.fontes?.find((f) => f.url)?.url) })
      }
    }
    return saida
  } catch {
